...
class AnimatedGIF(tk.Label):
    def _init_(self, master, paths, audio_path):
        self.master = master
        self.sequences = []
        self.delay = 100
    ...
    def open_new_window(self):
        ...
        gif_path = "C:/Users/Athashree/bckground.gif"
        ...
        login_audio_path = "C:/Users/Athashree/loginaudio.mp3"
        pygame.mixer.init()
        ...
        def login():
            ...
            if username == fixed_username and password == fixed_password:
                new_window.destroy()  # Close login window if login successful
                open_third_screen()  # Open third screen after successful login
            else:
...
def open_third_screen():
    third_window = tk.Toplevel(root)
    third_window.title("Third Screen")
    third_window.attributes('-fullscreen', True)

    gif_label = AnimatedGIF(third_window, ["C:/Users/Athashree/waves1.gif"])  # type: ignore # Change path accordingly
    gif_label.pack(fill=tk.BOTH, expand=True)

    gif_label.play_count = 0
    gif_label.show_next_frame()

    # Display live time and date in the top right corner with padding
    clock_label = tk.Label(third_window, text="", font=("Tekton Pro", 24), bg="navy", fg="white", highlightthickness=0)
    clock_label.place(relx=0.95, rely=0.1, anchor="ne")  # Place in top right corner but a little lower and to the left
    update_clock(clock_label)  # Start updating the clock

def update_clock(label):
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    current_date = now.strftime("%D-%m-%Y")
    label.config(text=f"{current_date}\n{current_time}")  # Set background to system default for transparency
    label.after(1000, update_clock, label)  # Update every second
...
gif_label = AnimatedGIF(root,["D:/PROGRAMMING/PYTHON/AIGENT/initgif.gif", "D:/PROGRAMMING/PYTHON/AIGENT/a.gif"] , "D:\PROGRAMMING\PYTHON\AIGENT\why1.mp3" ) # type: ignore
gif_label.pack(fill=tk.BOTH, expand=True)

gif_label.play_cnt = 0
gif_label.show_next_frame()
