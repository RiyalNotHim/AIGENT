...
classNames = []
classFile = "D:\PROGRAMMING\PYTHON\AIGENT\coco_demo"
with open(classFile, 'rt') as f:
    classNames = f.read().rstrip('\n').split('\n')

# Load the pre-trained model
configPath = 'D:\PROGRAMMING\PYTHON\AIGENT\ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt'
weightsPath = 'D:\PROGRAMMING\PYTHON\AIGENT\frozen_inference_graph.pb'

net1 = cv2.dnn.DetectionModel(weightsPath, configPath)
try:
    net1 = cv2.dnn.DetectionModel(weightsPath, configPath)
...
def update():
    ...
    if is_detection_running:
        classIds, confs, bbox = net1.detect(img, confThreshold=thres)
