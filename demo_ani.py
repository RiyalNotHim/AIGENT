...
class AnimatedGIF(tk.Label):
    ...
    def load_frames(self, path):
        ...
        resized_gif = gif.resize((1280, 720), Image.LANCZOS)  # Resize to 720p
        sequence = []
        try:
            while True:
                gif.seek(len(sequence))
                frame = resized_gif.copy()
                frame.load()
                sequence.append(ImageTk.PhotoImage(frame))
        except EOFError:
            pass
    ...
    def open_new_window(self):
        ...
        resized_gif = gif_image.resize((1280, 720), Image.LANCZOS)  # Resize to 720p
...
def open_third_screen():
    ...
    gif_label = AnimatedGIF(third_window, ["D:/PROGRAMMING/PYTHON/AIGENT/waves1.gif"])  
...
gif_label = AnimatedGIF(root, ["D:/PROGRAMMING/PYTHON/AIGENT/initgif.gif", "D:/PROGRAMMING/PYTHON/AIGENT/aig.gif"])  
gif_label.pack(fill=tk.BOTH, expand=True)
